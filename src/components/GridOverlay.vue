        ...
        <div class="grid-overlay__inner">
            ...
            <div class="grid-rows" :style="rowsStyle">
                ...
                <template v-if="effectiveRowCount > 0">
                    <div v-for="row in effectiveRowCount" :key="`row-${row}`" class="grid-row"
                        :style="{ ...rowStyle, height: rowHeightComputed }"></div>
                </template>
            </div>
            <!-- Contenedor para las columnas -->
            <div class="grid-columns" :style="columnsStyle">
                <div v-for="column in columnCount" :key="`col-${column}`" class="grid-column" :style="columnStyle">
                </div>
            </div>
            <div class="grid-baseline" :style="baselinStyle" v-if="baseline">
                <div v-for="baseline in baselineCount" :key="`baseline-${baseline}`" :style="baselineStyle"></div>
            </div>
                ...
                <div class="grid-controls__header">
                    <div class="grid-controls__selector">
                        <select v-model="selectedInput">
                            <option v-for="(input, index) in inputs" :key="`input-${index}`" :value="input.value">
                                {{ input.text }}
                            </option>
                        </select>
                    </div>
                    <div class="grid-controls__toggle">
                        <button @click="toggleControls"><svg width="16" height="16" viewBox="0 0 16 16" fill="none"
                    ...
                    <div class="row">
                        ...
                        <div class="control-group">
                            ...
                            <div class="input-percentage-wrapper">
                                <input type="number" id="row-opacity" min="0" max="100" v-model.number="rowOpacity" />
                    ...
                    <div class="row">
                        <div class="control-group">
                            ...
                            <select id="row-type" v-model="rowType" class="capitalize">
                                <option v-for="(type, index) in rowTypes" :key="index" :value="type">{{ type }}</option>
                            </select>
                        </div>
                        <div class="control-group">
                            <label for="row-height">Height</label>
                            <input type="text" id="row-height" v-model="rowHeight" :disabled="rowType === 'stretch'"
                                class="capitalize" />
                        </div>
                        <div class="control-group">
                            <label for="row-offset">Offset</label>
                            <input type="number" id="row-offset" min="0" max="100" v-model.number="rowOffset" />
                        </div>
                    </div>
                    <div class="row">
                        <div class="control-group">
                            <label for="row-gutter">Gutter</label>
                            <input type="number" id="row-gutter" min="0" max="100" v-model.number="rowGutter" />
                        </div>
                    </div>
                </div>
                <!-- Columns -->
                <div class="grid-controls__body columns" v-if="selectedInput === 'columns'">
                    <div class="row">
                        <div class="control-group">
                        ...
                        <div class="control-group">
                            ...
                            <div class="input-percentage-wrapper">
                                <input type="number" id="column-opacity" min="0" max="100"
                    ...
                    <div class="row">
                        <div class="control-group">
                            ...
                            <select id="column-type" v-model="columnType" class="capitalize">
                                <option v-for="(type, index) in columnTypes" :key="index" :value="type">{{ type }}
                                </option>
                            </select>
                        </div>
                        <div class="control-group">
                            <label for="column-width">Width</label>
                            <input type="text" id="column-width" v-model="columnWidth"
                                :disabled="columnType === 'stretch'" class="capitalize" />
                        </div>
                        <div class="control-group">
                            <label for="column-margin">Margin</label>
                            <input type="number" id="column-margin" min="0" max="100" v-model.number="columnMargin" />
                        </div>
                    </div>
                    <div class="row">
                        <div class="control-group">
                            <label for="column-gutter">Gutter</label>
                            <input type="number" id="column-gutter" min="0" max="100" v-model.number="columnGutter" />
                        </div>
                    </div>
                </div>
                <div class="grid-controls__body baseline" v-if="selectedInput === 'baseline'">
                    <div class="row">
                        <div class="control-group">
                            <label for="baseline">Baseline</label>
                            <div class="basline-px-wrapper">
                                <input type="number" id="baseline" min="0" max="100" v-model.number="baseline" />
                        ...
                        <div class="control-group">
                            ...
                            <div class="input-percentage-wrapper">
                                <input type="number" id="baseline-opacity" min="0" max="100"
...
const effectiveRowCount = computed(() => {
    ...
    if (rowCount.value === 'auto') {
        ...
        const totalRowsPossible = Math.floor((viewportHeight + rowGutter.value) / (rowHeightInt + rowGutter.value));
...
const rowHeightComputed = computed(() => {
    ...
    // Caso para 'top', 'center', 'bottom' con 'auto'
        ...
        if (rowCount.value === 'auto' || !rowCount.value) {
            ...
            const totalRowsPossible = Math.floor((viewportHeight + rowGutter.value) / (estimatedRowHeight + rowGutter.value));
            const heightPerRow = (viewportHeight - (rowGutter.value * (totalRowsPossible - 1))) / totalRowsPossible;
            return `${heightPerRow}px`;
        } else {
...
watch(() => rowType.value, (newValue) => {
    if (newValue === 'stretch') {
        rowHeight.value = 'auto';
    }
});
watch(() => columnType.value, (newValue) => {
    if (newValue === 'stretch') {
...
onMounted(() => {
    if (isClient) {
        ...
        columnOpacity.value = parseInt(localStorage.getItem('columnOpacity'), 10) || 10;
        ...
        columnMargin.value = parseInt(localStorage.getItem('columnMargin'), 10) || 0;
        rowOffset.value = parseInt(localStorage.getItem('rowOffset'), 10) || 0;
        columnGutter.value = parseInt(localStorage.getItem('columnGutter'), 10) || 8;
        ...
        baselineOpacity.value = parseInt(localStorage.getItem('baselineOpacity'), 10) || 10;
